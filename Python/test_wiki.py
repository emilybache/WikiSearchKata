...
def test_create_pages():
    root_page = WikiPage(title="RootPage", text="some text on the root page", tags=["foo", "bar"])
    child_page = WikiPage(title="Child1", text="a child page", tags=["foo"])
    root_page.add_child(child_page)
    assert root_page.title == "RootPage"
    assert "Child1" in map(lambda x: x.title, root_page.children)
    assert "RootPage" in map(lambda x: x.title, child_page.parents)
    
    
def test_uri():
    root_page = WikiPage(title="RootPage", text="some text on the root page", tags=["foo", "bar"], uri="/")
    child_page = WikiPage(title="Child1", text="a child page", tags=["foo"])
    root_page.add_child(child_page)
